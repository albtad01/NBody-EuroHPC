...
#include "mipp.h"
#include "SimulationNBodyOpenMP_Green.hpp"
...
template <typename T>
SimulationNBodyOpenMP_Green<T>::SimulationNBodyOpenMP_Green(const BodiesAllocatorInterface<T>& allocator, const T soft)
...
template <typename T>
void SimulationNBodyOpenMP_Green<T>::initIteration()
{
    #pragma omp parallel for schedule(static)
    for (unsigned long i = 0; i < this->getBodies()->getN(); i++) {
        this->accelerations[i] = accAoS_t<T>{0, 0, 0};
    }
}

template <typename T>
const std::vector<accAoS_t<T>>& SimulationNBodyOpenMP_Green<T>::getAccAoS() {
    return accelerations;
}

template <typename T>
void SimulationNBodyOpenMP_Green<T>::computeBodiesAcceleration()
{
    ...
    for (long i = 0; i < n; i++) {
        ...
        for (long j = 0; j < n_vec; j += V) {
            ...
            const mipp::Reg<T> r_dist2 = mipp::fmadd(r_dx, r_dx, mipp::fmadd(r_dy, r_dy, mipp::fmadd(r_dz, r_dz, r_soft)));
...
template <typename T>
void SimulationNBodyOpenMP_Green<T>::computeOneIteration()
{
    this->initIteration();
...
template class SimulationNBodyOpenMP_Green<float>;
template class SimulationNBodyOpenMP_Green<double>;