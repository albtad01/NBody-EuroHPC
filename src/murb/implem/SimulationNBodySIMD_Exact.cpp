...
#include "mipp.h"
#include "SimulationNBodySIMD_Exact.hpp"
...
template <typename T>
SimulationNBodySIMD_Exact<T>::SimulationNBodySIMD_Exact(const BodiesAllocatorInterface<T>& allocator, const T soft)
...
template <typename T>
void SimulationNBodySIMD_Exact<T>::initIteration()
{
    std::fill(this->accelerations.begin(), this->accelerations.end(), accAoS_t<T>{T(0), T(0), T(0)});
}

template <typename T>
const std::vector<accAoS_t<T>>& SimulationNBodySIMD_Exact<T>::getAccAoS() {
    return accelerations;
}

template <typename T>
void SimulationNBodySIMD_Exact<T>::computeBodiesAcceleration()
{
...
template <typename T>
void SimulationNBodySIMD_Exact<T>::computeOneIteration()
{
    this->initIteration();
...
template class SimulationNBodySIMD_Exact<float>;
template class SimulationNBodySIMD_Exact<double>;
