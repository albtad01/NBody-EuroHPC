...
#include "mipp.h"
#include "SimulationNBodyOpenMP_Exact.hpp"
...
template <typename T>
SimulationNBodyOpenMP_Exact<T>::SimulationNBodyOpenMP_Exact(const BodiesAllocatorInterface<T>& allocator, const T soft)
...
template <typename T>
void SimulationNBodyOpenMP_Exact<T>::initIteration()
{
    std::fill(this->accelerations.begin(), this->accelerations.end(), accAoS_t<T>{T(0), T(0), T(0)});
}

template <typename T>
const std::vector<accAoS_t<T>>& SimulationNBodyOpenMP_Exact<T>::getAccAoS() {
    return accelerations;
}

template <typename T>
void SimulationNBodyOpenMP_Exact<T>::computeBodiesAcceleration()
{
    ...
    // srun -p iml-ia770 -n 1 --cpus-per-task=12 --threads-per-core=1 --cpu-bind=cores bash -lc ' export OMP_NUM_THREADS=$SLURM_CPUS_PER_TASK export OMP_PLACES=cores export OMP_PROC_BIND=close export OMP_SCHEDULE=static export OMP_WAIT_POLICY=ACTIVE ./bin/murb -n 30000 -i 200 -v --nv --im cpu+omp'
    // srun -p iml-ia770 -n 1 --cpus-per-task=12 --threads-per-core=1 --cpu-bind=cores \
        bash -lc 'export OMP_NUM_THREADS=$SLURM_CPUS_PER_TASK; export OMP_PLACES=cores; export OMP_PROC_BIND=close; export OMP_SCHEDULE=static; export OMP_WAIT_POLICY=ACTIVE; ./bin/murb -n 30000 -i 200 -v --nv --im cpu+omp'
    #ifdef _OPENMP
    ...
    for (long i = 0; i < n; i++) {
        ...
        // Scalar tail: avoid pow(), match vector math (inv^3).
        for (long j = n_vec; j < n; j++) {
            ...
            // Use sqrt + reciprocal; keep it consistent with SIMD path.
...
template <typename T>
void SimulationNBodyOpenMP_Exact<T>::computeOneIteration()
{
    this->initIteration();
...
template class SimulationNBodyOpenMP_Exact<float>;
template class SimulationNBodyOpenMP_Exact<double>;
